...
//
// Functions for processing sentences from a corpus that consist of an array of tokens
//

// Returns true if the rule applies at site. As a side effect it assigns the new
// category to testTag
TransformationRule.prototype.isApplicableAt = function(sentence, i) {
  // Convert sentence to the right format for the tagger
  ...
  var applies = this.predicate.evaluate(taggedSentence, i);
...
TransformationRule.prototype.applyAt = function(sentence, i) {
  var taggedSentence = sentence.map(function(token) {
    var pair = [token.token, token.tag];
    return pair;
  });
