...
Brill_POS_Trainer.prototype.scoreRule = function(rule, i, j) {
  ...
  if (rightTag !== oldTag) {
    ...
    if (newTag === rightTag) {
      ...
      logger.debug("Brill_POS_Trainer.prototype.scoreRule: positive: " + rule.key() + "\t score: " + rule.score.positive);
    }
    else {
      // New tag is wrong as well --> neutral
      rule.neutral++;
      logger.debug("Brill_POS_Trainer.prototype.scoreRule: neutral: " + rule.key() + "\t score: " + rule.score.neutral);
    }
  }
  else {
    // Old tag is right
    if (newTag === rightTag) {
      ...
      rule.negative++;
      rule.hasBeenSelectedAsHighRuleBefore = false;
      logger.debug("Brill_POS_Trainer.prototype.scoreRule: negative: " + rule.key() + "\t score: " + rule.score.negative);
    }
  }
...
Brill_POS_Trainer.prototype.scanForPositiveRules = function() {
  ...
  this.corpus.sentences.forEach(function(sentence, i) {
    sentence.forEach(function(token, j) {
      ...
      that.generatePositiveRules(i, j);
    });
  });
