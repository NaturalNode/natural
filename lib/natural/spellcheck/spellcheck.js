...
function Spellcheck(wordlist) {
    ...
    for(var i in this.wordlist) {
        if(!this.word2frequency[this.wordlist[i]]) {
            this.word2frequency[this.wordlist[i]] = 0;
        }
        this.word2frequency[this.wordlist[i]]++;
    }
}

Spellcheck.prototype.isCorrect = function(word) {
    return this.wordlist.indexOf(word) > -1;
}

Spellcheck.prototype.getCorrections = function(word) {
    return editsUpToDistance([word], 1).filter(function(word) { return this.wordlist.indexOf(word) > -1; })
                       .map(function(word) { return [word, this.word2frequency[word]]; })
